...
mapping(address => bool) public jurisdictionBanned;
...
modifier checkJurisdiction() {
    require(!jurisdictionBanned[msg.sender], "Jurisdiction not allowed");
    _;
}

// ==== Compliance Admin Functions ====
function setKYCStatus(address user, bool status) external onlyOwner {
    kycPassed[user] = status;
}

function setJurisdictionStatus(address user, bool banned) external onlyOwner {
    jurisdictionBanned[user] = banned;
}
...
function book(
    ...
    uint8 months,
    string calldata tokenURI_
) external nonReentrant onlyKYCed checkJurisdiction returns (uint256) {
